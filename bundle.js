...
require('./js/Keys.js');
var MapControls = require('./js/MapControls.js').MapControls;
var Selection = require('./js/Selection.js').Selection;
var Util = require('./js/Util.js').Util;
var Models = require('./js/Models.js').Models;
var UnitType = require('./js/UnitType.js').UnitType;
var Debug = require('./js/Debug.js').Debug;
var ModelLoader = require('./js/ModelLoader.js').ModelLoader;
...
function worldToScreen(pos) {
  var vector = pos.project(game.scene.camera);
...
function SkyBox() {
  ...
  var mesh = new THREE.Mesh(new THREE.BoxGeometry(boxSize, boxSize, boxSize), skyBoxMaterial);
...
function getSampler() {
  return new Promise(function (resolve, reject) {
    ...
    loader.load('models/maps/man.jpg', function (image) {
      ...
      sampler.getHeight = function (x, y) {
...
function initGround() {
  ...
  var groundGeometry = new THREE.PlaneBufferGeometry(config.terrain.width, config.terrain.height, config.terrain.xFaces, config.terrain.yFaces);
  groundGeometry.applyMatrix(new THREE.Matrix4().makeRotationX(Math.PI / -2));
  for (var i = 0; i <= config.terrain.xFaces; i++) {
    game.heightField[i] = [];
  }
  var stride = 3;
  var positionsLength = undefined;
  ...
  for (var i = 0; i < positionsLength; i += stride) {
    var x = groundGeometry.attributes.position.array[i];
    var z = groundGeometry.attributes.position.array[i + 2];
    var y = 0;
    // let edges of map be 0
    var eps = 0.01;
    if (x > -config.terrain.width / 2 + eps && x < config.terrain.width / 2 - eps && z > -config.terrain.height / 2 + eps && z < config.terrain.height / 2 - eps) {
      var noise = game.noiseGen.noise(x / 100, z / 100);
      // normalize [-1,1] to [0,1]
      var normalNoise = (noise + 1) / 2;
      y = normalNoise * config.terrain.maxElevation + config.terrain.minElevation;
      groundGeometry.attributes.position.array[i + 1] = y;
    }

    var xi = (x + config.terrain.width / 2) * config.terrain.xFaces / config.terrain.width;
    var yi = (z + config.terrain.height / 2) * config.terrain.yFaces / config.terrain.height;
  ...
  samplerPromise.then(function (sampler) {
    for (var i = 0; i < positionsLength; i += 3) {
      var x = groundGeometry.attributes.position.array[i];
      var z = groundGeometry.attributes.position.array[i + 2];
      var xt = (x + config.terrain.width / 2) / config.terrain.width;
      var yt = (z + config.terrain.height / 2) / config.terrain.height;
      var xi = Math.round(xt * config.terrain.xFaces);
      var yi = Math.round(yt * config.terrain.yFaces);
      var y = sampler.getHeight(xt, yt) + config.terrain.minElevation;
...
function Sea() {
  var plane = new THREE.PlaneGeometry(config.terrain.width * 2, config.terrain.height * 2, 1, 1);
  ...
  var seaMesh = new THREE.Mesh(plane, material);
  ...
  this.render = function () {
    var endTime = new Date().getTime() / 1000.0;
    ...
    var angv = game.scene.camera.position.clone();
    // angv.normalize();
    angv.applyQuaternion(game.scene.camera.quaternion);
    material.uniforms.angv.value = angv;
    var ori = new THREE.Vector3(4, 5.5, 5.0).multiplyScalar(10.0);
...
function resetCamera() {
  ...
  var x = game.scene.scene3.position.x;
  var z = game.scene.scene3.position.z;
...
function initCameraControls() {
  ...
  game.scene.cameraControls = new MapControls(game.scene.camera, game.scene.navigationPlane, function () {
    return null;
  }, game.scene.renderer.domElement, resetCamera);
...
function getGroundHeight(x, y) {
  var xd = (x + config.terrain.width / 2) * config.terrain.xFaces / config.terrain.width;
  var yd = (y + config.terrain.height / 2) * config.terrain.yFaces / config.terrain.height;
  var xi = Math.floor(xd);
  var yi = Math.floor(yd);

  // https:// en.wikipedia.org/wiki/Bilinear_interpolation
  var x1 = x - xd % 1 * config.terrain.width / config.terrain.xFaces;
  var x2 = x1 + 1 * config.terrain.width / config.terrain.xFaces;
  var y1 = y - yd % 1 * config.terrain.height / config.terrain.yFaces;
  var y2 = y1 + 1 * config.terrain.height / config.terrain.yFaces;
  if (xi < 0 || yi < 0 || xi >= game.heightField.length || xi + 1 >= game.heightField.length || yi >= game.heightField[xi].length || yi + 1 >= game.heightField[xi].length) {
    return 0;
  }
  var fQ11 = game.heightField[xi][yi];
  var fQ21 = game.heightField[xi + 1][yi];
  var fQ12 = game.heightField[xi][yi + 1];
  var fQ22 = game.heightField[xi + 1][yi + 1];
...
function getGroundAlignment(unit) {
  var groundHeight = getGroundHeight(unit.position.x, unit.position.z);
  var y = groundHeight - unit.bbox.min.y * unit.scale.y;
...
function moveAlignedToGround(object) {
  ...
  var oldTime = object.lastMoved;
  var timeDelta = nowTime - oldTime;
  ...
  var speed = config.units.speed * object.moveSpeed * timeDelta;
  var zAxis = new THREE.Vector3(0, 0, speed);
  zAxis.applyQuaternion(object.quaternion);
  zAxis.y = 0;
  var xzDirection = zAxis;
  var oldGroundHeight = getGroundAlignment(object);
  ...
  var groundHeight = object.position.y;

  // rotate in velocity direction
  var dir = xzDirection.clone();
...
function Missiles() {
  ...
  var material = game.models.missile.material;
  var geometry = game.models.missile.geometry;
  ...
  var maxInstances = config.effects.missilePool;
  Util.createAttributeMatrix(igeo, maxInstances);

  var vertexShader = $('#generic-vertex').text();
  var fragmentShader = $('#generic-fragment').text();
  ...
  var imesh = new THREE.Mesh(igeo, newMaterial);
  addToScene(imesh);
  ...
  this.updateMissiles = function () {
    var focus = getCameraFocus(0, 0);
    imesh.position.copy(focus);
    for (var missileId in this.active) {
      if (this.active.hasOwnProperty(missileId)) {
        var missile = this.active[missileId];
...
function initialPlaceUnit(unit, size) {
  ...
  var height = size.height * unit.scale.y;
  //const groundHeight = getGroundHeight(unit.position.x, unit.position.z);
  var groundHeight = getGroundAlignment(unit);
...
function UnitPool() {
  ...
  for (var i = 0; i < config.units.maxUnits; i++) {
...
function createM3Unit(modelOptions, instance) {
  ...
  var size = getSize(modelOptions.bboxHelper.box);
  ...
  try {
    for (var _iterator = instance.geomats[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      var geomat = _step.value;

      var _geomat = _slicedToArray(geomat, 2);

      var geo = _geomat[0];
      var _mat = _geomat[1];

      var a_teamColor = geo.getAttribute('a_teamColor');
      var c = TeamColors[unit.team].clone().multiplyScalar(255);
  ...
  var boxMesh = new THREE.Mesh(new THREE.BoxGeometry(size.height, size.width, size.depth), new THREE.MeshLambertMaterial({ color: 0xFFFFFF, transparent: true, opacity: 0.5 }));
  var mat2 = new THREE.Matrix4().makeRotationX(modelOptions.rotation.x);
  boxMesh.geometry.applyMatrix(mat2);
  var mat3 = new THREE.Matrix4().makeRotationZ(modelOptions.rotation.z);
  boxMesh.geometry.applyMatrix(mat3);
  var mat = new THREE.Matrix4().makeTranslation((unit.bbox.min.x + unit.bbox.max.x) / 2, (unit.bbox.min.y + unit.bbox.max.y) / 2, (unit.bbox.min.z + unit.bbox.max.z) / 2);
  ...
  try {
    for (var _iterator2 = unit.children[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
  ...
  unit.createNew = function () {
    var instancePromise = game.m3loader.addInstance(modelOptions);
    return new Promise(function (resolve, reject) {
      instancePromise.then(function (obj3d) {
        var unit = createM3Unit(modelOptions, obj3d);
        resolve(unit);
      });
    });
  };

  unit.removeUnit = function () {
    var _iteratorNormalCompletion3 = true;
    var _didIteratorError3 = false;
    var _iteratorError3 = undefined;

    try {
      for (var _iterator3 = unit.geomats[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
        var geomat = _step3.value;

        var _geomat2 = _slicedToArray(geomat, 2);

        var geo = _geomat2[0];
        var _mat2 = _geomat2[1];
...
function createM3Units() {
  var m3loader = game.m3loader;

  var models = m3loader.getModels();
  ...
  try {
    var _loop = function () {
      ...
      for (var i = 0; i < config.units.m3count; i++) {
        var instancePromise = m3loader.addInstance(model);
        instancePromise.then(function (obj3d) {
          var unit = createM3Unit(model, obj3d);
...
function HealthBars() {
  ...
  var healthMaterial = new THREE.RawShaderMaterial({
    uniforms: {},
    attributes: ['position', 'a_health'],
    transparent: true,
    vertexShader: $('#health-vertex').text(),
    fragmentShader: $('#health-fragment').text()
  });
  var geo = new THREE.BufferGeometry();
  var maxInstances = config.units.maxUnits;
  geo.addAttribute('position', new THREE.BufferAttribute(new Float32Array(maxInstances * 3), 3));

  var healthAttribute = new THREE.BufferAttribute(new Float32Array(maxInstances), 1);
  geo.addAttribute('a_health', healthAttribute);

  var mesh = new THREE.PointCloud(geo, healthMaterial);
  addToScene(mesh);
  ...
  this.updateHealthBars = function (units) {
    ...
    try {
      for (var _iterator5 = units[Symbol.iterator](), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
        var unit = _step5.value;

        var position = unit.position.clone().sub(focus);
...
function TeamBars() {
  ...
  var teamMaterial = new THREE.RawShaderMaterial({
    uniforms: {},
    attributes: ['position', 'a_color'],
    transparent: true,
  ...
  var colorAttribute = new THREE.BufferAttribute(new Float32Array(maxInstances * 3), 3);
  geo.addAttribute('a_color', colorAttribute);

  var mesh = new THREE.PointCloud(geo, teamMaterial);
  addToScene(mesh);
  ...
  this.updateTeamBars = function (units) {
    ...
    try {
      for (var _iterator6 = units[Symbol.iterator](), _step6; !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = true) {
        ...
        var color = TeamColors[unit.team];
...
function createUnit(options) {
  ...
  var boxMaterial = options.boxMaterial;
  var bboxHelper = options.bboxHelper;
  var size = options.size;

  var materialClone = material.clone();
  var unit = new THREE.Mesh(geometry, materialClone);
  unit.model = options;
...
function loadModels(finishCallback) {
  function getLoadTextureSuccess(options, material, units) {
    return function (texture) {
      ...
      try {
        for (var _iterator7 = units[Symbol.iterator](), _step7; !(_iteratorNormalCompletion7 = (_step7 = _iterator7.next()).done); _iteratorNormalCompletion7 = true) {
  ...
  function getOnSuccess(options) {
    ...
    return function (geometry) {
      ...
      var material = new THREE.MeshLambertMaterial({ color: 0xF5F5F5, transparent: true, opacity: opacity });

      // for showing bounding box
      var boxMaterial = new THREE.MeshLambertMaterial({ color: 0x0000FF, opacity: 0.0, transparent: true });
      var proto = new THREE.Mesh(geometry, material.clone());
      addToScene(proto);
      var bboxHelper = new THREE.BoundingBoxHelper(proto, 0);
      bboxHelper.update();
      removeFromScene(proto);
      var size = getSize(bboxHelper.box);
      var boxGeometry = new THREE.BoxGeometry(size.height, size.width, size.depth);
      var _iteratorNormalCompletion8 = true;
      var _didIteratorError8 = false;
      var _iteratorError8 = undefined;

      try {
        for (var _iterator8 = boxGeometry.vertices[Symbol.iterator](), _step8; !(_iteratorNormalCompletion8 = (_step8 = _iterator8.next()).done); _iteratorNormalCompletion8 = true) {
      ...
      var units = [];
      if (options.type === UnitType.Air || options.type === UnitType.Ground) {
        for (var i = 0; i < config.units.count; i++) {
          var unit = createUnit(options);
          units.push(unit);
          game.units.push(unit);
        }
      ...
      textureLoader.load(texturePath, getLoadTextureSuccess(options, material, units), getLoadTextureProgress(options));
    };
  }
  ...
  try {
    for (var _iterator10 = Models[Symbol.iterator](), _step10; !(_iteratorNormalCompletion10 = (_step10 = _iterator10.next()).done); _iteratorNormalCompletion10 = true) {
      var model = _step10.value;

      var modelOptions = $.extend({}, options, model);
      ...
      loader.load(model.path, getOnSuccess(modelOptions), getOnProgress(modelOptions));
    }
  } catch (err) {
...
function getBBoxes() {
  ...
  try {
    for (var _iterator11 = game.units[Symbol.iterator](), _step11; !(_iteratorNormalCompletion11 = (_step11 = _iterator11.next()).done); _iteratorNormalCompletion11 = true) {
      var unit = _step11.value;

      var pos = unit.position;
      var bbox = unit.bbox.clone();
      // rotate bounding box with object
      var mat = new THREE.Matrix4().makeRotationFromQuaternion(unit.quaternion);
      bbox.applyMatrix4(mat);
      var box = [pos.x + bbox.min.x * unit.scale.x, pos.y + bbox.min.y * unit.scale.y, pos.z + bbox.min.z * unit.scale.z, pos.x + bbox.max.x * unit.scale.x, pos.y + bbox.max.y * unit.scale.y, pos.z + bbox.max.z * unit.scale.z];
      box.unit = unit;
      boxes.push(box);
    }
  } catch (err) {
...
function mark(unit) {
  var size = getSize(unit.bbox);
  var radius = Math.max(size.height, Math.max(size.width, size.depth)) / 2;
...
function initSelection() {
  var mouseElement = game.scene.renderer.domElement;
...
function initDAT() {
  ...
  for (var varName in config) {
    if (config.hasOwnProperty(varName)) {
      var inner = config[varName];
      if (isObject(inner)) {
        var folder = gui.addFolder(varName);
        for (var varName2 in inner) {
          if (inner.hasOwnProperty(varName2)) {
            var controller = folder.add(inner, varName2);
            controller.listen();
        ...
        var controller = gui.add(config, varName);
        controller.listen();
  ...
  controllers['camera.mouseControl'].onChange(function () {
    game.scene.cameraControls.enabled = config.camera.mouseControl;
  });
...
function updateUnitInfo() {
  if (game.selector.selected.length > 0) {
    var unit = game.selector.selected[0];
    var x = formatFloat(unit.position.x);
    var y = formatFloat(unit.position.y);
    var z = formatFloat(unit.position.z);
    var min = unit.bbox.min;
    var minx = formatFloat(min.x);
    var miny = formatFloat(min.y);
    var minz = formatFloat(min.z);
    var max = unit.bbox.max;
    var maxx = formatFloat(max.x);
    var maxy = formatFloat(max.y);
    var maxz = formatFloat(max.z);
    var health = formatFloat(unit.health);
    var s = '<table>';
...
function updateCameraInfo() {
  var x = game.scene.camera.position.x;
  var y = game.scene.camera.position.y;
  var z = game.scene.camera.position.z;
  var xr = game.scene.camera.rotation.x;
  var yr = game.scene.camera.rotation.y;
  var zr = game.scene.camera.rotation.z;
  config.camera.X = x;
  config.camera.Y = y;
  config.camera.Z = z;
...
function updateBBoxes() {
  ...
  try {
    for (var _iterator12 = game.units[Symbol.iterator](), _step12; !(_iteratorNormalCompletion12 = (_step12 = _iterator12.next()).done); _iteratorNormalCompletion12 = true) {
...
function funTerrain() {
  var time = new Date().getTime() / 1000.0 - game.startTime;
  for (var i = 0; i < game.scene.ground.geometry.attributes.position.length; i += 3) {
    var x = game.scene.ground.geometry.attributes.position.array[i];
    var z = game.scene.ground.geometry.attributes.position.array[i + 2];
    var noise = game.noiseGen.noise3d(x / 100, z / 100, time);
    var normalNoise = (noise + 1) / 2;
    var y = normalNoise * config.terrain.maxElevation;
...
function getCameraFocus(mouseX, mouseY) {
  ...
  var mesh = game.scene.navigationPlane;
...
function MiniMap() {
  var minimapHeight = config.dom.controlsHeight - 2;
  ...
  var $renderElement = $(minimapRenderer.domElement);
  $renderElement.css({
    border: '1px solid white'
  });
  var minimapScene = new THREE.Scene();
  var minimapCamera = new THREE.PerspectiveCamera(35, minimapWidth, minimapHeight, 0, 1000);
  ...
  var minimapMaterial = new THREE.PointCloudMaterial({ size: 0.1 });
  var light = new THREE.AmbientLight(0xFFFFFF);
  minimapScene.add(light);

  var oldCloud = undefined;
  var mouseDown = false;
  var oldCameraRectMesh = undefined;

  var cameraRectMaterial = new THREE.MeshLambertMaterial({
    wireframe: false,
    color: 0x00FF00,
    transparent: true,
    opacity: 0.5
  ...
  function translate(pos) {
    var v = new THREE.Vector3(pos.x * 2 / config.terrain.width, 0, pos.z * 2 / config.terrain.height);
    return v;
  }
  ...
  this.render = function () {
    ...
    try {
      for (var _iterator13 = game.units[Symbol.iterator](), _step13; !(_iteratorNormalCompletion13 = (_step13 = _iterator13.next()).done); _iteratorNormalCompletion13 = true) {
        var unit = _step13.value;

        if (unit.minimap === undefined) {
          unit.minimap = {};
        }
        var v = translate(unit.position);
        geom.vertices[i] = v;
        i++;
      }
    } catch (err) {
    ...
    var pointCloud = new THREE.PointCloud(geom, minimapMaterial);
    minimapScene.add(pointCloud);
    oldCloud = pointCloud;

    var cameraRectGeometry = new THREE.PlaneGeometry(1, 1, 1, 1);
    ...
    var cameraRectMesh = new THREE.Mesh(cameraRectGeometry, cameraRectMaterial);
  ...
  function setPos(evt) {
    var x = evt.offsetX / $renderElement.width() * 2 - 1;
    var y = -evt.offsetY / $renderElement.height() * 2 + 1;

    // swapping x and z because of camera rotation
    var mapZ = -x * config.terrain.width / 2;
    var mapX = -y * config.terrain.height / 2;

    var camera = game.scene.camera;

    var focus = getCameraFocus(0, 0);
    var offset = camera.position.clone().sub(focus);
  ...
  $renderElement.mouseenter(function (evt) {
    game.scene.cameraControls.edgeScrollEnabled = false;
  });
  $renderElement.mouseleave(function (evt) {
    game.scene.cameraControls.edgeScrollEnabled = true;
  });
}
...
function render() {
  ...
  */
  ...
  game.components.forEach(function (x) {
    x.render();
  });
...
function checkCollisions() {
  ...
  boxIntersect(boxes, function (i, j) {
    var p1 = boxes[i].unit.position;
    var p2 = boxes[j].unit.position;
    var d = p2.clone();
    // clearLog();
    // log(i, j, p1, p2, d);
    d.sub(p1);
    ...
    var p1new = p1.clone().sub(d);
    var p2new = p2.clone().add(d);
    if (p1new.x >= game.mapBounds.min.x && p1new.x <= game.mapBounds.max.x && p1new.z >= game.mapBounds.min.z && p1new.z <= game.mapBounds.max.z) {
      p1.sub(d);
    }
    if (p2new.x >= game.mapBounds.min.x && p2new.x <= game.mapBounds.max.x && p2new.z >= game.mapBounds.min.z && p2new.z <= game.mapBounds.max.z) {
      p2.add(d);
    }
  });
}
...
function findTargets() {
  ...
  try {
    for (var _iterator14 = game.units[Symbol.iterator](), _step14; !(_iteratorNormalCompletion14 = (_step14 = _iterator14.next()).done); _iteratorNormalCompletion14 = true) {
      var unit = _step14.value;

      var pos = unit.position.toArray();
      pos.unit = unit;
      points.push(pos);
    }
  } catch (err) {
  ...
  try {
    var _loop2 = function () {
      ...
      var minHealth = Number.POSITIVE_INFINITY;
      unit.attackTarget = null;
      tree.rnn(unit.position.toArray(), unit.weapon.range, function (idx) {
        var target = points[idx].unit;
    ...
    for (var _iterator15 = game.units[Symbol.iterator](), _step15; !(_iteratorNormalCompletion15 = (_step15 = _iterator15.next()).done); _iteratorNormalCompletion15 = true) {
      _loop2();
    }
  } catch (err) {
...
function Explosions() {
  var texture = THREE.ImageUtils.loadTexture('models/images/explosion.png');
  texture.anisotropy = game.scene.renderer.getMaxAnisotropy();
  ...
  var newpool = [];
  for (var i = 0; i < config.effects.explosionPool; i++) {
    var mesh = this.createExplosion();
    newpool.push(mesh);
  }
  this.pool = newpool;
}
  ...
  return function () {
    ...
    if (target.health <= 0 && oldHealth > 0) {
      (function () {
        ...
        target = null;
        var explosion = game.explosions.createExplosion();
        explosion.position.copy(pos);
        explosion.time = 0;
        explosion.tscale = 0;
        explosion.opacity = 2;
        explosion.material.uniforms.time.value = _this.time;
        //explosion.material.uniforms.tExplosion.value = game.explosion;
        addToScene(explosion);
        var seed = Math.random();
        game.sound.blast();
        var tween = new TWEEN.Tween(explosion).to({ time: 1, tscale: 1, opacity: 0 }, 1000).onUpdate(function () {
          this.material.uniforms.time.value = this.time + seed;
          this.material.uniforms.opacity.value = this.opacity;
          var scale = this.tscale;
...
function attackTargets() {
  ...
  try {
    for (var _iterator16 = game.units[Symbol.iterator](), _step16; !(_iteratorNormalCompletion16 = (_step16 = _iterator16.next()).done); _iteratorNormalCompletion16 = true) {
      var unit = _step16.value;

      if (unit.attackTarget !== null) {
        (function () {
          var time = getGameTime();
          var target = unit.attackTarget;
          if (unit.lastShot === undefined || unit.lastShot + unit.weapon.reload < time) {
            (function () {
              var shot = {
                delta: 0
              };
              game.sound.shot();
              var shotMesh = game.missiles.createMissile();
              var startPos = unit.position.clone();
              var tween = new TWEEN.Tween(shot).to({
                delta: 1
              }, 500).onUpdate(function () {
                var diff = target.position.clone().sub(startPos).multiplyScalar(this.delta);
                diff.add(startPos);
...
function removeDead() {
  game.units = game.units.filter(function (unit) {
    if (unit.dead) {
      unit.removeUnit();
      var newUnitPromise = unit.createNew();
      newUnitPromise.then(function (newUnit) {
        game.newUnits.push(newUnit);
      });
    }
    return !unit.dead;
  });
  var toAdd = game.newUnits;
...
function updateSimulation() {
  ...
  try {
    for (var _iterator17 = game.units[Symbol.iterator](), _step17; !(_iteratorNormalCompletion17 = (_step17 = _iterator17.next()).done); _iteratorNormalCompletion17 = true) {
      var unit = _step17.value;
...
function initM3Models() {
  ...
  var $modal = $('#m3progress');
  var $modalBody = $modal.find('.modal-body');
  var sources = {};
  $modal.modal();

  var progress = function progress(evt) {
    // function handles progress events of two types:
    // from Viewer.addEventListener('load') and from THREE.CompressedTextureLoader
    var source = evt.target.source || evt.target.responseURL;
    if (sources[source] === undefined) {
      var _$progress = $('#m3unit.progress').clone();
      _$progress.attr('id', '');
      _$progress.removeClass('invisible');
      $modalBody.append(_$progress);
      sources[source] = {
        $progress: _$progress
      };
    }
    var $progressParent = sources[source].$progress;
    var $progress = sources[source].$progress.find('.progress-bar');
...
function initScene() {
  ...
  var frustumNear = 1;
  game.scene.camera = new THREE.PerspectiveCamera(35, game.scene.width / game.scene.height, frustumNear, frustumFar);
...
},{"./js/Debug.js":2,"./js/Keys.js":3,"./js/MapControls.js":4,"./js/ModelLoader.js":5,"./js/Models.js":6,"./js/Selection.js":7,"./js/UnitType.js":8,"./js/Util.js":9,"bootstrap":11,"box-intersect":24,"howler":38,"jquery":39,"static-kdtree":40,"tween.js":63}],2:[function(require,module,exports){
'use strict';
...
var Debug = {
  ...
  debugLoadMarkers: function debugLoadMarkers(scene) {
  ...
  drawOutLine: function drawOutLine(units, worldToScreen) {
    ...
    try {
      for (var _iterator = units[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        ...
        var geometry = unit.bboxMesh.geometry.clone();
        var mat = new THREE.Matrix4().makeRotationFromQuaternion(unit.quaternion);
        geometry.applyMatrix(mat);
        ...
        try {
          for (var _iterator2 = geometry.vertices[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
            ...
            var pos = vertex.clone().multiply(unit.scale).add(unit.position);
            var vec2 = worldToScreen(pos);
        ...
        var div = unit.outline;
        if (div === undefined) {
          div = $('<div/>');
          unit.outline = div;
          viewport.append(div);
        }
        var left = screenBox.min.x;
        var _top = screenBox.min.y;
        var width = screenBox.max.x - screenBox.min.x;
        var height = screenBox.max.y - screenBox.min.y;
        div.css({
          position: 'absolute',
          left: left,
          top: _top,
...
function MapControls(camera, mesh, renderFunction, domElement, resetCamera) {
  ...
  function arcBallRotation(mul) {
    var updown = arguments.length <= 1 || arguments[1] === undefined ? false : arguments[1];
    ...
    if (intersects.length > 0) {
      ...
      if (updown) {
        ...
        camera.position.copy(_focus).add(offset);
      } else {
        axis.y = 0;
        axis.normalize();
...
var M3Models = require('./m3/M3Models.js');
var Util = require('./Util.js').Util;
var mpqFile = M3Models.mpqFile;

function ModelLoader(options) {
  ...
  function createObject(modelOptions, model, geomats) {
    var rotation = modelOptions.rotation;
    var scale = modelOptions.scale;
    ...
    try {
      for (var _iterator = geomats[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        var geomat = _step.value;
  ...
  this.addInstance = function (modelOptions) {
    ...
    var model = modelOptions.model;
    var onLoad = function onLoad(object) {
      var instance = object.instance;
      var teamId = 0;
      var modelInfo = scope.modelByName[model.model.name];
      var meshparent = createObject(modelOptions, model, modelInfo.geomats);
    ...
    var promise = new Promise(function (resolve, reject) {
      var listener = function listener(object) {
      ...
      var asyncInstance = model.instances[0];
  ...
  this.addModel = function (modelOptions, model, viewer, progress) {
    ...
    var uvSetCount = model.model.uvSetCount;
    // console.log("batchlength", batches.length);
    var l2 = batches.length;

    var geomats = [];
    var pTextures = [];
    var pTexturesBySource = {};

    // make a big hwbones texture for the whole animation
    var context = viewer.getContext();
    // in milliseconds
    var asyncinstance = model.instances[0];
    var instance = asyncinstance.instance;
    var skeleton = instance.skeleton;
    var hwbones = skeleton.hwbones;
    var hwbonesLength = hwbones.length;

    var sequenceId = model.model.sequencesByName[modelOptions.sequence];
    ...
    var animationLength = model.model.sequences[instance.sequence].animationEnd;
    var frames = Math.round(animationLength / context.frameTime);
    // console.log("frames", frames, model.model.name);
    var bighwbones = new Float32Array(frames * hwbonesLength);
    ...
    var hwTextureWidth = instance.hwTextureWidth;
    var hwTextureHeight = instance.hwTextureHeight;
    instance.bighwbones = bighwbones;
    var hwbonesTexture = new THREE.DataTexture(bighwbones, instance.hwTextureWidth, instance.hwTextureHeight, THREE.RGBAFormat, THREE.FloatType);
    ...
    var boneAtIndex = function boneAtIndex(index) {
      var offset = index * 16;
      var elements = [];
    ...
    var updatePositions = function updatePositions() {
      ...
      for (var i = 0; i < l; i++) {
        ...
        for (var k = 0; k < region.elements; k++) {
          ...
          var weight = new THREE.Vector4(bytes[byteIndex + 12], bytes[byteIndex + 13], bytes[byteIndex + 14], bytes[byteIndex + 15]);

          var bone = new THREE.Vector4(bytes[byteIndex + 16], bytes[byteIndex + 17], bytes[byteIndex + 18], bytes[byteIndex + 19]);

          // debug code
          var weightedBone0 = boneAtIndex(bone.x + boneLookup).multiplyScalar(weight.x / 255);
          var weightedBone1 = boneAtIndex(bone.y + boneLookup).multiplyScalar(weight.y / 255);
          var weightedBone2 = boneAtIndex(bone.z + boneLookup).multiplyScalar(weight.z / 255);
          var weightedBone3 = boneAtIndex(bone.w + boneLookup).multiplyScalar(weight.w / 255);
          var position = new THREE.Vector4(pos.x, pos.y, pos.z, 1.0);
          var outposition4 = new THREE.Vector4();
          ...
          var outposition = new THREE.Vector3(outposition4.x, outposition4.y, outposition4.z);
    ...
    var bgeo = updatePositions();
    // END UPDATEPOSITIONS

    var maxInstances = options.maxInstancesPerModel;
    ...
    var _loop = function (i) {
      ...
      for (var k = 0; k < region.elements; k++) {
        ...
        var normal = new THREE.Vector4(bytes[byteIndex + 20], bytes[byteIndex + 21], bytes[byteIndex + 22], bytes[byteIndex + 23]);
        normals.push(normal);

        for (var uvi = 0; uvi < uvSetCount; uvi++) {
          var uvx = shorts[shortIndex + 24 / 2 + 0 + uvi / 2];
          var uvy = shorts[shortIndex + 24 / 2 + 1 + uvi / 2];
          var uvPos = new THREE.Vector2(uvx, uvy);
          uvs[uvi].push(uvPos);
        }
        // set unused uvis as well, to avoid switching GL programs
        for (var uvi = uvSetCount; uvi < 4; uvi++) {
          var uvx = shorts[shortIndex + 24 / 2 + 0 + 0 / 2];
          var uvy = shorts[shortIndex + 24 / 2 + 1 + 0 / 2];
          var uvPos = new THREE.Vector2(uvx, uvy);
          uvs[uvi].push(uvPos);
        }

        var tangent = new THREE.Vector4(bytes[byteIndex + 24 + uvSetCount * 4], bytes[byteIndex + 25 + uvSetCount * 4], bytes[byteIndex + 26 + uvSetCount * 4], bytes[byteIndex + 27 + uvSetCount * 4]);
      ...
      var setLayerSettings = function setLayerSettings(layer, layerSettings) {
        for (var name in layerSettings) {
          var fullName = layer.uniforms[name];
          material.uniforms[fullName] = layerSettings[name];
        }
      };
      var layers = sourceMaterial.layers;
      var b2i = function b2i(b) {
        return b ? 1 : 0;
      };
        ...
        var _loop2 = function () {
          ...
          if (layer.active) {
            ...
            if (pTexturesBySource[layer.source] !== undefined) {
              pTexturesBySource[layer.source].then(function (tinfo) {
              ...
              var promise = new Promise(function (resolve, reject) {
                loader.load(mpqFile(layer.source), function (texture) {
      ...
      for (var attributeName in material.attributes) {
        var attribute = material.attributes[attributeName];
        ...
        var attributeArray = new Float32Array(value.length * size);
        var _i = 0;
        ...
        try {
          for (_iterator3 = value[Symbol.iterator](); !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
            ...
            var arval = val.toArray();
            for (var j = 0; j < size; j++) {
              attributeArray[_i + j] = arval[j];
            }
            _i += size;
          }
        } catch (err) {
      ...
      var teamColorsAttribute = new THREE.InstancedBufferAttribute(new Float32Array(maxInstances * 3), 3, 1, true);
      geo.addAttribute('a_teamColor', teamColorsAttribute);
      var frameAttribute = new THREE.InstancedBufferAttribute(new Float32Array(maxInstances), 1, 1, true);
      ...
      if (modelOptions.layers !== undefined) {
        if (modelOptions.layers.filter(function (l) {
          return l == i;
        }).length === 0) {
          return 'continue';
        };
    ...
    var pTexture = Promise.all(pTextures);

    var pReturn = new Promise(function (resolve, reject) {
      pTexture.then(function () {

        //const meshparent = createObject(modelOptions, model, geomats);
        var meshparent = new THREE.Mesh(bgeo, new THREE.MeshBasicMaterial());
        options.scene.add(meshparent);
        var bboxHelper = new THREE.BoundingBoxHelper(meshparent, 0);
        bboxHelper.update();
        ...
        try {
          for (var _iterator4 = geomats[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
            var geomat = _step4.value;
            ...
            var sceneMesh = new THREE.Mesh(geo, mat);
  ...
  this.loadModels = function (progress) {
    ...
    try {
      for (var _iterator5 = M3Models.M3Models[Symbol.iterator](), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
    ...
    var waitForIt = function waitForIt(resolve) {
      var ready = true;
    ...
    var onLoad = function onLoad(resolve) {
      ...
      try {
        for (var _iterator6 = models[Symbol.iterator](), _step6; !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = true) {
          ...
          try {
            for (var _iterator8 = model.model.sequences[Symbol.iterator](), _step8; !(_iteratorNormalCompletion8 = (_step8 = _iterator8.next()).done); _iteratorNormalCompletion8 = true) {
              var sequence = _step8.value;

              model.model.sequencesByName[sequence.name] = _i2;
              _i2++;
            }
          } catch (err) {
            _didIteratorError8 = true;
      ...
      try {
        for (var _iterator7 = models[Symbol.iterator](), _step7; !(_iteratorNormalCompletion7 = (_step7 = _iterator7.next()).done); _iteratorNormalCompletion7 = true) {
          var model = _step7.value;

          var modelOptions = modelOptionsByName[model.model.name];
          modelOptions.model = model;
          var promise = scope.addModel(modelOptions, model, viewer, progress);
      ...
      var modelPromise = Promise.all(modelPromises);
      ...
      modelPromise.then(function () {
        ...
        var instancePromise = Promise.all(instancePromises);
        instancePromise.then(resolve);
      });
    };

    var allModelsLoaded = new Promise(function (resolve, reject) {
      viewer.addEventListener('load', function () {
        return waitForIt(resolve);
      });
    });
    ...
    var update = function update() {
      ...
      var elapsed = nowTime - oldTime;
      var FPS = 60;
      var elapsedFrames = elapsed * FPS;
      oldTime = nowTime;
      scope.instanceRegister = scope.instanceRegister.filter(function (unit) {
        if (unit.dead) {
          var _iteratorNormalCompletion9 = true;
          var _didIteratorError9 = false;
          var _iteratorError9 = undefined;
          ...
          try {
            for (var _iterator9 = unit.geomats[Symbol.iterator](), _step9; !(_iteratorNormalCompletion9 = (_step9 = _iterator9.next()).done); _iteratorNormalCompletion9 = true) {
              var geomat = _step9.value;

              var _geomat3 = _slicedToArray(geomat, 2);

              var geo = _geomat3[0];
              var mat = _geomat3[1];

              var a_mv_matrix = new THREE.Matrix4().makeTranslation(Number.POSITIVE_INFINITY, Number.POSITIVE_INFINITY, Number.POSITIVE_INFINITY);
      ...
      var focus = options.getCameraFocus(0, 0);
      for (var _i3 = 0; _i3 < scope.instanceRegister.length; _i3++) {
        var instance = scope.instanceRegister[_i3];
        for (var j = 0; j < instance.geomats.length; j++) {
          var geomat = instance.geomats[j];
          var geo = geomat[0];
          var mat = geomat[1];
          var mesh = geomat.localMesh;

          geomat.sceneMesh.position.copy(focus);

          var a_mv_matrix = new THREE.Matrix4();
          ...
          var a_frame = geo.getAttribute('a_frame');
...
},{"./Util.js":9,"./m3/M3Models.js":10}],6:[function(require,module,exports){
'use strict';
...
},{"./UnitType.js":8}],7:[function(require,module,exports){
'use strict';
...
function Selection(options) {
  ...
  var selectionY = options.config.terrain.maxElevation;

  // plane for intersecting mouse cursor to select units
  var selectionPlaneGeometry = new THREE.PlaneBufferGeometry(options.config.terrain.width, options.config.terrain.height, 1, 1);
  selectionPlaneGeometry.applyMatrix(new THREE.Matrix4().makeRotationX(Math.PI / -2));
  var selectionPlaneMaterial = new THREE.MeshLambertMaterial({ color: 0xFFFFFF });
  var selectionPlane = new THREE.Mesh(selectionPlaneGeometry, selectionPlaneMaterial);
  selectionPlane.visible = false;
  selectionPlane.position.y = selectionY;
  options.scene.add(selectionPlane);

  var selectionRectangleGeometry = new THREE.PlaneBufferGeometry(1, 1, 1, 1);
  selectionRectangleGeometry.applyMatrix(new THREE.Matrix4().makeRotationX(Math.PI / -2));
  var selectionRectangleMaterial = new THREE.MeshLambertMaterial({ color: 0x00FF00, opacity: 0.5, transparent: true });
  var selectionRectangle = new THREE.Mesh(selectionRectangleGeometry, selectionRectangleMaterial);
  ...
  function getScreenBoxes() {
    ...
    try {
      for (var _iterator = options.game.units[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        var unit = _step.value;
        ...
        try {
          for (var _iterator2 = geometry.vertices[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
            ...
            var pos = vertex.clone().multiply(unit.scale).add(unit.position);
            var vec2 = options.worldToScreen(pos);
            screenBox.expandByPoint(vec2);
          }
        } catch (err) {
          _didIteratorError2 = true;
        ...
        var box = [screenBox.min.x, screenBox.min.y, screenBox.max.x, screenBox.max.y];
        box.unit = unit;
  ...
  function checkIntersectScreen(x1, y1, x2, y2) {
    var screenBoxes = getScreenBoxes();
    var flatSelectionBox = [x1, y1, x2, y2];
    var selectedIndices = options.boxIntersect(screenBoxes, [flatSelectionBox]);
    var _iteratorNormalCompletion3 = true;
    var _didIteratorError3 = false;
    var _iteratorError3 = undefined;

    try {
      for (var _iterator3 = selection.selected[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
        var s = _step3.value;
    ...
    try {
      for (var _iterator4 = selectedIndices[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
        var _step4$value = _slicedToArray(_step4.value, 2);

        var i = _step4$value[0];
        var j = _step4$value[1];

        var unit = screenBoxes[i].unit;
        options.mark(unit);
  ...
  this.getOnMouseDown = function () {
    return function (eventData) {
      if (eventData.which === leftMouseButton) {
        ...
        try {
          for (var _iterator5 = selection.selected[Symbol.iterator](), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
            var s = _step5.value;
      ...
      if (eventData.which === rightMouseButton) {
        ...
        var intersects = raycaster.intersectObject(options.ground);
        if (intersects.length > 0 && selection.selected.length > 0) {
          ...
          try {
            for (var _iterator6 = selection.selected[Symbol.iterator](), _step6; !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = true) {
              var unit = _step6.value;
  ...
  function rectangleSelect(evt) {
    ...
    if ($selectionDiv.css('visibility') === 'visible') {
      var x1 = $selectionDiv.startPos[0];
      var y1 = $selectionDiv.startPos[1];
      var x2 = evt.clientX;
      var y2 = evt.clientY;
      if (x2 < x1) {
        var _ref = [x2, x1];
  ...
  var mouseElement = options.mouseElement;

  var handler = selection.getOnMouseMove(mouseElement);
  $(mouseElement).mousemove(handler);

  var downHandler = selection.getOnMouseDown();
...
},{"jquery":39}],8:[function(require,module,exports){
'use strict';
...
var Util = {
  ...
  createAttributeMatrix: function createAttributeMatrix(geo, maxInstances) {
    geo.addAttribute('a_mv0', new THREE.InstancedBufferAttribute(new Float32Array(maxInstances * 4), 4, 1, true));
    geo.addAttribute('a_mv1', new THREE.InstancedBufferAttribute(new Float32Array(maxInstances * 4), 4, 1, true));
    geo.addAttribute('a_mv2', new THREE.InstancedBufferAttribute(new Float32Array(maxInstances * 4), 4, 1, true));
    geo.addAttribute('a_mv3', new THREE.InstancedBufferAttribute(new Float32Array(maxInstances * 4), 4, 1, true));
  },

  updateAttributeMatrix: function updateAttributeMatrix(geo, matrix, index) {
    var a_mv = [];
    ...
    for (var k = 0; k < a_mv.length; k++) {
      a_mv[k].setXYZW(index, matrix.elements[k * 4 + 0], matrix.elements[k * 4 + 1], matrix.elements[k * 4 + 2], matrix.elements[k * 4 + 3]);
...
try {
  for (var _iterator = M3Models[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
...
function mpqFile(path) {
  if (path === "ground.png" || path === "sky.png" || path === "water.png") {
    var newPath = CORSPROXY + "http://viewer.hiveworkshop.com/images/" + path;
    return newPath;
  }
...
},{"./lib/intersect":26,"./lib/sweep":30,"typedarray-pool":33}],25:[function(require,module,exports){
'use strict'
...
},{"./brute":25,"./median":27,"./partition":28,"./sweep":30,"bit-twiddle":31,"typedarray-pool":33}],27:[function(require,module,exports){
'use strict'
...
},{"./sort":29,"bit-twiddle":31,"typedarray-pool":33}],31:[function(require,module,exports){
/**
...
},{"bit-twiddle":31,"buffer":34,"dup":32}],34:[function(require,module,exports){
/*!
...
},{"./lib/heap.js":41,"bit-twiddle":42,"inorder-tree-layout":43,"ndarray":58,"ndarray-ops":45,"ndarray-pack":50,"ndarray-scratch":56,"ndarray-select":57,"typedarray-pool":62}],41:[function(require,module,exports){
"use strict"
...
},{"typedarray-pool":62}],42:[function(require,module,exports){
...
},{"cwise-compiler":46}],46:[function(require,module,exports){
"use strict"
...
},{"./lib/thunk.js":48}],47:[function(require,module,exports){
"use strict"
...
},{"uniq":49}],48:[function(require,module,exports){
"use strict"
...
},{"./compile.js":47}],49:[function(require,module,exports){
"use strict"
...
},{"./doConvert.js":51,"ndarray":58}],51:[function(require,module,exports){
...
module.exports = generateCWiseOp
},{"uniq":55}],54:[function(require,module,exports){
"use strict"
...
},{"bit-twiddle":42,"buffer":34,"dup":33}],63:[function(require,module,exports){
/**
